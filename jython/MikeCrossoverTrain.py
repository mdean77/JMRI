...
class StartCrossoverTrain(jmri.jmrit.automat.AbstractAutomaton):
    ...
    def handle(self):
        ...
        print("EAST sensor 13 inactive so now safe to set cross over for next lap")
        self.crossOver.setRoute()
        print("Cross over route should be established")
        print("Now we are just making a full lap until engine approaches mountain")
        print("Waiting for east 10 to become active")
        self.waitSensorActive([self.eastSensor10])
        print("Sensor 10 is active")
        print("Ready to start up hill - will slow down")
        self.throttle.setSpeedSetting(0.2)
        print("Speed set to 20%")
        self.LongHorn(1200,800,2)
        print("Now on the mountain and it sensor should be active")
        print("Waiting for eastern 10 to be inactive ")
        self.waitSensorInactive([self.eastSensor10])
        print("Sensor 10 inactive")
        print("Waiting till I am off the mountain before speed up")
        self.waitSensorInactive([self.mountainSensor])
        print("Mountain sensor inactive")
        print("My train is off the mountain route - can speed up")
        self.throttle.setSpeedSetting(0.25)
        print("Speed is 25% - still have to go through interchange switches")
        self.waitSensorActive([self.eastSensor13])
        print("East 13 active and entering northern zone")
        self.LongHorn(1200,800,2)
        self.waitSensorInactive([self.eastSensor13])
        print("Increasing speed to 35%")
        self.throttle.setSpeedSetting(0.35)
        print("East 13 inactive so safe to set straight thru route back")
        self.straightThru.setRoute()
        print("Straight thru should be set")
